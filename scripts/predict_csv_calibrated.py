...
run_dir, in_csv, text_col, max_len, out_csv, T = sys.argv[1:]; max_len=int(max_len); T=float(T)
...
with torch.inference_mode():
    for i in range(0, len(texts), 32):
        ...
        p = (clf(**enc).logits / T).softmax(dim=-1)[:,1].cpu().numpy(); preds.extend(p)
df["pred_cal"] = np.array(preds); df.to_csv(out_csv, index=False)
